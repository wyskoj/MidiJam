...
GLboolean Ms3dBundle::ApplyMaterial() {
    ...
    for (short i = 0; i < this->nMaterials + 1; ++i) {
        if (this->materialVisibility[i] == 1) {
            int material_index = i - 1;
            if (material_index >= 0) {
...
// MATCH: EXACT
void Ms3dBundle::ApplyTextures(FILE* hwfStream, void* hwfAppendix, unsigned int hwfAppendixEntriesCount) {
    int i; // [esp+4h] [ebp-4h]

    for (i = 0; i < this->nMaterials; ++i) {
        if (strlen(this->pMaterials[i].pTextureName) > 0)
        ...
        if (strlen(this->pMaterials[i].pAlphaMapName) > 0)
...
{
    for (int m = 0; m < this->nMaterials + 1; ++m) {
        if (this->materialVisibility[m] == 1) {
            this->nTrianglesPerMaterial[m] = this->BuildVertexBuffer(
                &this->pRenderState[m], m);
            this->pRenderState[m].displayListId = glGenLists(1);
            if (!this->pRenderState[m].displayListId)
                return;
            glNewList(this->pRenderState[m].displayListId, GL_COMPILE);
            glEnableClientState(GL_VERTEX_ARRAY);
            glEnableClientState(GL_TEXTURE_COORD_ARRAY);
            glEnableClientState(GL_NORMAL_ARRAY);
            glVertexPointer(3, GL_FLOAT, sizeof(InterleavedVertex), this->pRenderState[m].vertexBuffer);
            glTexCoordPointer(
                2,
                GL_FLOAT,
                sizeof(InterleavedVertex),

                this->pRenderState[m].vertexBuffer->texCoord);
            glNormalPointer(GL_FLOAT, sizeof(InterleavedVertex),
                            this->pRenderState[m].vertexBuffer->normal);
            glDrawArrays(GL_TRIANGLES, 0, 3 * this->nTrianglesPerMaterial[m]);
...
// ReSharper enable CppDFANullDereference
...
// MATCH: EXACT
void Ms3dBundle::RenderCompiled() const {
    for (short i = 0; i < nMaterials + 1; ++i) {
