...
void RenderStageString() {
    ...
    for ( i = 0; i < g_stageStringCount; ++i )
    {
        if ( g_stageString[i].isActive == 1 )
        {
            for ( j = 0; j < 12; ++j )
            {
                glPushMatrix();
                ...
                if ( g_stageString[i].field_4[j] <= 0 )
                {
                    z = STAGE_BASE_Z - 2.0;
                ...
                else
                {
                    ...
                    ya = (double)g_stageString[i].field_4[j] / (double)g_stageString[i].field_34[j] * -8.0 + 4.0;
...
bool UpdateStageString(MUSIC_TIME pmtNow) {
    ...
    for ( i = 0; i < g_stageStringCount; ++i )
    {
        ...
        for ( note = 0; note < 12; ++note )
        {
            ...
            {
                if ( g_stageString[i].field_364[note][slot] > 0 )
                {
                    v6 = 1;
                    g_stageString[i].field_364[note][slot] -= pmtNow - g_currentGlobalTime;
                    if ( g_stageString[i].field_364[note][slot] <= 0 )
                    {
                        g_stageString[i].field_364[note][slot] = 0;
                        g_stageString[i].field_4[note] = g_stageString[i].field_64[note][slot];
                        g_stageString[i].field_34[note] = g_stageString[i].field_64[note][slot];
                        g_stageString[i].field_64[note][slot] = 0;
                    }
                    g_stageString[i].isActive = 1;
                    g_stageString[i].timeToLive = 1000;
                    v2 = 1;
                }
            }
            if ( g_stageString[i].field_4[note] > 0 )
            {
                g_stageString[i].field_4[note] -= pmtNow - g_currentGlobalTime;
                if ( g_stageString[i].field_4[note] > 0 )
                {
                    g_stageString[i].isActive = 1;
                    g_stageString[i].timeToLive = 1000;
                ...
                {
                    g_stageString[i].field_4[note] = 0;
                }
            }
        }
        if ( !v2 && g_stageString[i].isActive == 1 && --g_stageString[i].timeToLive <= 0 )
        {
            g_stageString[i].timeToLive = 0;
            g_stageString[i].isActive = 0;
        }
    }
    return v6 == 1;
}
