...
void RenderXylophone() {
  ...
  for ( i = 0; i < g_xylophoneCount; ++i )
  {
    ...
    for ( note = 0; note < 88; ++note )
    {
      ...
      {
        if ( g_xylophone[i].queue[note][slot] > 0 && g_xylophone[i].queue[note][slot] < v17 )
          v17 = g_xylophone[i].queue[note][slot];
      }
      if ( v17 <= 0 || v17 >= 300.0 )
      {
...
LABEL_31:
      ...
      if ( KEY_SHAPE_INDEX[note] >= 9 && KEY_SHAPE_INDEX[note] <= 13 )
      {
        glTranslatef(KEY_WIDTHS[KEY_SHAPE_INDEX[note]], 0.0, 2.0);
        if ( g_xylophone[i].field_6[note] <= 0.0 )
...
bool UpdateXylophone(MUSIC_TIME pmtNow) {
  ...
  for ( i = 0; i < g_xylophoneCount; ++i )
  {
    ...
    for ( note = 0; note < 88; ++note )
    {
      for ( slot = 0; slot < 16; ++slot )
      {
        if ( g_xylophone[i].queue[note][slot] > 0 )
        {
          v6 = 1;
          g_xylophone[i].queue[note][slot] -= pmtNow - g_currentGlobalTime;
          if ( g_xylophone[i].queue[note][slot] <= 0 )
          {
            g_xylophone[i].queue[note][slot] = 0;
            g_xylophone[i].field_6[note] = g_xylophone[i].field_18C6[note][slot] * 8.0 / 128.0 + 1.75;
            g_xylophone[i].field_B6[note] = g_xylophone[i].field_2C6[note][slot];
            g_xylophone[i].field_2C6[note][slot] = 0;
          }
          g_xylophone[i].isActive = 1;
          g_xylophone[i].timeToLive = 1000;
          v2 = 1;
        }
      }
      if ( g_xylophone[i].field_B6[note] <= 0 )
      {
        if ( g_xylophone[i].field_6[note] > 0.0 )
        {
          v6 = 1;
      ...
      {
        g_xylophone[i].field_B6[note] -= pmtNow - g_currentGlobalTime;
        if ( g_xylophone[i].field_B6[note] <= 0 )
          g_xylophone[i].field_B6[note] = 0;
        g_xylophone[i].isActive = 1;
        g_xylophone[i].timeToLive = 1000;
        v2 = 1;
    ...
    if ( !v2 && g_xylophone[i].isActive == 1 && --g_xylophone[i].timeToLive <= 0 )
    {
      g_xylophone[i].timeToLive = 0;
      g_xylophone[i].isActive = 0;
    }
  }
  return v6 == 1;
}
